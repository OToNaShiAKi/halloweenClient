<template>
  <v-main>
    <v-img
      eager
      :class="current >= index.bless ? 'animate__backInDown' : 'hide'"
      class="animate__animated"
      src="../assets/banner.png"
    />
    <v-img
      eager
      :class="current >= index.bless ? 'animate__bounceIn' : 'hide'"
      class="mx-15 animate__animated animate__delay-2s"
    ...
    <section
      v-if="current >= index.present"
      class="animate__animated animate__fadeInLeft text-center"
    >
...
import { Birthday } from "../store/request";
...
export default {
  ...
  filters: {
    filter(selected) {
      let tip = "";
      if (selected.length >= 4) return "小孩子才做选择！我全都要！";
